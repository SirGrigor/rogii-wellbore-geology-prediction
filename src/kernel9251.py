"""Faithful port of the public LB-9.251 ROGII kernel's feature engine.

Source: Kaggle notebook `nihilisticneuralnet/9-251-rogii-wellbore-geology-prediction-dwt-based`
(treat as Apache-2.0; credit to the author). This is the verbatim feature-engineering code
(particle filters with momentum, multi-scale/stochastic DTW, beam search, multi-scale NCC,
GR affine calibration, formation/ANCC spatial imputers). Adapted ONLY: the CFG-coupled
module-level imputer fit is replaced by fit_imputers(well_ids, data_dir); no other changes.

We port it to reproduce ~9.25 on our sacred holdout (a measured target), then innovate on
top per docs/roadmap_to_9.5.md. build_well/build_dataset work unchanged on our data layout.
"""
...
def fit_imputers(well_ids, data_dir):
    """Fit the formation-plane + dense-ANCC spatial imputers; set module globals.
    Fit on DEV wells for a clean sacred gate, or on all train wells for the test build."""
    global _FI, _DI
    _FI = FormationPlaneKNN(list(well_ids), Path(data_dir))
    _DI = DenseANCCImputer(list(well_ids), Path(data_dir))
